...
#figure(caption: [$C_V$ scores for tested hyperparameter sets], placement: auto, {
  set text(size: 11pt)
  table(
    columns: (auto, auto, auto, 15%),
    table.header([No. of neighbours (UMAP)], [No. of components (UMAP)], [Minimum cluster size (HDBSCAN)], [$C_v$]),

    [30], [10], [30], [#calc.round(0.6260656565788427, digits: 3)],

    [15], [10], [30], [#calc.round(0.6251453237522169, digits: 3)],

    [30], [5], [30], [#calc.round(0.6247880561567395, digits: 3)],

    [30], [5], [20], [#calc.round(0.6218670777506773, digits: 3)],

    [30], [10], [20], [#calc.round(0.6178856581242598, digits: 3)],

    [15], [10], [10], [#calc.round(0.6138945314312856, digits: 3)],

    [15], [5], [30], [#calc.round(0.6073894745143839, digits: 3)],

    [30], [5], [10], [#calc.round(0.6059289829719166, digits: 3)],

    [15], [5], [10], [#calc.round(0.6038504287879352, digits: 3)],

    [15], [10], [20], [#calc.round(0.601119136530049, digits: 3)],

    [15], [5], [20], [0.600],
    // [#calc.round(0.6004166027278458, digits: 3)],

    [30], [10], [10], [#calc.round(0.5957065404077899, digits: 3)],
  )
}) <table-cv>
...
#figure(
  ...
  table(
    ...
    [1], [1688], [tools, generative, artificialintelligence, intelligence, artificial, robots, ai, future, humans, art],

    [], [848], [gpt3, essay, teacher, programming, telugu, referral, autobiography, instructgpt, freud, method],

    [], [476], [seo, search, engines, engine, searches, query, links, results, keywords, ads],

    [], [473], [students, education, classroom, schools, ban, cheating, teachers, kids, educators, teacher],

    [2],
    [1858],
    [generative, tools, artificialintelligence, revolution, future, jobs, artists, ethics, intelligence, machinelearning],

    [], [1121], [gpt3, telugu, teacher, programming, raw, hell, essay, referral, crazy, autobiography],

    [], [646], [seo, search, engines, searches, keywords, web, threat, ads, results, query],

    [], [571], [students, teachers, cheating, education, ban, classroom, student, essay, teaching, class],

    [3], [1660], [generative, tools, artificialintelligence, robots, mind, ai, intelligence, future, jobs, humans],

    [], [807], [gpt3, telugu, essay, programming, teacher, referral, instructgpt, freud, step, wallet],

    [], [513], [students, education, classroom, schools, ban, cheating, teacher, educators, teachers, student],

    [], [501], [seo, search, engines, engine, searches, threat, ads, keywords, query, results],
  ),
) <table-topics>
...
#grid(
  columns: 2,
  align: center + bottom,
  [#figure(caption: [Intertopic distance map], rect(image(
      "graphics/intertopic-distance.png",
      width: 85%,
    ))) <fig-intertopic-dist>],
  [#figure(caption: [Topic similarity matrix], image(
      "graphics/similarity-matrix.png",
      width: 90%,
...
#figure(caption: [Word clouds for the ten most frequent topics], image("graphics/wordclouds.png")) <fig-wordclouds>
